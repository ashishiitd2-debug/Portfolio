// Preloader
        ...
        function initTimelineRider() {
            const timeline = document.querySelector('.timeline-line');
            const rider = document.getElementById('timeline-rider');
            const roadSvg = timeline ? timeline.querySelector('.timeline-road') : null;
            const roadPath = document.getElementById('timeline-road-path');
            if (!timeline || !rider || !roadPath || !roadSvg) return;

            const centerRiderOnPath = (progress) => {
                const clamped = Math.max(0, Math.min(1, progress));
                const total = roadPath.getTotalLength();
                const pt = roadPath.getPointAtLength(total * clamped);
                const timelineRect = timeline.getBoundingClientRect();
                const roadRect = roadSvg.getBoundingClientRect();

                const roadOffsetLeft = roadRect.left - timelineRect.left;
                const roadOffsetTop = roadRect.top - timelineRect.top;
                const pathX = roadOffsetLeft + (pt.x / 120) * roadRect.width;
                const pathY = roadOffsetTop + (pt.y / 1000) * roadRect.height;
            ...
            ScrollTrigger.create({
                ...
                onUpdate: (self) => centerRiderOnPath(self.progress),
                onRefresh: (self) => centerRiderOnPath(self.progress)
            });
        }
